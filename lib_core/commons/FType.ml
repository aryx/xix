...
(* Helpers *)
(*****************************************************************************)
(* alt: port semgrep-libs Fpath_.exts but requires to port more of Fpath 
 * was in Filename_.ml before
*)

let regexp_match (s : string) (re : string) : string =
  assert (s =~ re);
  Str.matched_group 1 s

(* updated: added '-' in filesuffix because of file like foo.c-- *)
let filesuffix (s : string) : string =
  try regexp_match s ".+\\.\\([a-zA-Z0-9_-]+\\)$" with
  | _ -> "NOEXT"

let fileprefix (s : string) : string =
  try regexp_match s "\\(.+\\)\\.\\([a-zA-Z0-9_]+\\)?$" with
  | _ -> s
...
let of_file file =
  ...
  | "r" -> PL R
  | "ex" -> PL Elixir
(* TODO need port UFile  | _ when UFile.is_executable file -> Binary e *)
