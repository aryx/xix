...
** TODO changes.txt
fix o5c for helloc.exe

finish rc enough so can use it with mk on xix itself! remove dependency to 9base
 in Dockerfile and Dockerfile.light!

TODO Can use rc/mk on principia and build and syncweb as well Editor.nw/Codemap.nw
  need implement globbing, subshell, etc.

** Overview
# add c-- target to occ! so better than plan 9! factorized code
#  among 5c/6c/... once and for all, and reusable in OCaml too (and tiger)!
...
# then finish o5a/o5c/o5l (and ova/ovc/ovl and oia/oic/oil)
...
*** LATER FUN resume building and running orio (on Raspberry Pi? via ocaml-light plan9?)
# need first to resume ocaml-light for plan9

*** LATER resume building and running o9 kernel on Raspberry Pi and qemu-system-arm
...
*** LATER add wasm arch and assembler/linker/compiler for it!

*** LATER implement ziglang on top of xix assemblers/linkers
as an alternative to the C compiler. Zig as a better C.

Zix (Zig with xix) as an improvement over Zig itself too, depending
on just ocaml instead of depending on cmake, gcc/clang, llvm/lld/... like
the current ziglang.

Pretty cool language and improvement over C. Maybe can add to C incrementally?
 - EASY i32, u32, etc.
 - automatic overflow detection at runtime? (strict debug mode)
 - automatic runtime memory bug detection (leak, double free, etc.)
   integrating by default the best of valgrind
 - good stack traces
 - overflow ok with +%= 
 - error type ! and try/catch/...
 - option type and no null by default
 - pointer (not null), and slice for explicit pointer arithmetic
 - if/while/for as expression too, better switch syntax
 - better syntax for types []xx, not mixed left/right part
 - HARDER: comptime?
...
*** an orc -restricted like for ed, that disallows globbing!
good for AI coding agent to forbid dangerous rm -rf * :)

*** RELAX add a -profile for omk, orc, etc. with ppx_profiling

*** xix iwp9.nw refinments until March 9
fix omk/orc
add ohoc, oawk, osed
omothra? (use my html/css/js parser, resume meh and mmm?)
resume o9
improve oas/olk/occ/, add riscv?

*** bug: plan9: why can't I just run 'opwd' in /bin path?
I get a "no bytecode" error.
But if I do /bin/opwd it works, weird.

*** orc

**** TEDIOUS implement globbing! 
How is it implemented in C?

alt: reuse Glob library in semgrep? maybe not worth it for now and
good porting exercise to convert C code to OCaml.
update: reuse re_glob.ml from ocaml-re now in lib_core/regexps/re/

does not hurt though to have a look at the Glob library by Martin
in semgrep-libs

**** implement Subshell @{ for for loop, used heavily in xix mkfile
add test? test infra?

**** try use rc with mk! MKSHELL!
but need globbing first, lots of commands are doing rm *.o for example

*** => get orc/omk working enough that can compile xix, kencc, and plan9!
# actually I lie in the iwp9 paper and said it was already working

*** xix utilities

**** RELAX LP split hoc

**** RELAX port hoc to ocaml, skeleton

**** LP split awk

**** WAITING FUN port utilities/text/awk/ to OCaml
but first better split it and explain it in Utilities.nw
https://maximullaris.com/awk_tech_notes.html
https://maximullaris.com/awk.html

**** skeleton sed? do like ogrep, just mimimal program
copy code of oed?

**** LP split grep
and port to ocaml
even though already my grep v0 with just 100 LOC is already useful

**** LP split sed
and port to ocaml

*** LATER: resume o9, get it working again on qemu-system-arm like in mkfile
# can then talk about it in iwp9 paper!
compile and link o9 for raspeberry pi! see xix/kernel/mkfile 

*** misc

**** switch to COMP_EXTRA_FLAGS in mkcommon
use OCFLAGS, OLDFLAGS or OCFLAGS, OLFLAGS or O_CFLAGS O_LDFLAGS

**** remove main.8 from byterun/mkfile
anyway it's linked manually and can't work from a lib I think

*** testinfra/devinfra: fix Nix CI xix
also would be good to fix nix locally; I can't even compile
xix under nix-shell --pure on my thelioastra
pb when opam switch create 4.14.2 and in CI I have weird
linking error about dll_stdcompat_stub.so

*** still? omkrc, linked together, works?
global state to reset? big? should be simple! FP power!

then add discussion about it in xix paper!

hmm but lose //ism then, because here we fork every recipe
which is convenient because they execute in parallel.
so good only for NPROC=1, for more would need actually multicore.
hmm but can still get it if rc single-threaded but fork
programs, but pb is we waitpid for programs in mk, not rc.

*** xix and plan9

**** can have 'oed' running under plan9?
just comment callunix?
hmm Disk use Unix, so have to rewrite code to instead
use channel and just open each time you read or write
and use Pervasives seek

but pb is that need caps too at more places

**** less: remove deps to Unix in all of xix/lib_core/commons
CapUnix.ml and Proc.ml out in lib_system/?

**** less: get rid of deps to Unix (and unix.a) in commons/

**** less: remove deps to Str and dbe_of_filename stuff in Ftype.ml

*** LPize xix
# use Cursor and fill the explanations quickly! or use Claude Code in VScode?
# or the recent google antigravity? or extend my own efuns?

**** continue omk.nw

***** start use \ifocaml in Intro.nw to differentiate
like when taling about mk in C vs mk in OCaml and 5500 Lines of C ->
3300 lines of OCaml?

***** generate mk_help.tex automatically from ../mk binary!
and included it in section 2.2

**** continue orc.nw

*** --------------------------------------------------

*** cleanup todos/xix.txt, move stuff in ocaml-light/todo.txt too

*** forbid use of stdout, stdin, stderr in semgrep.jsonnet

*** omk on xix/ does not work well when NPROC=64
weird unbound module because I think ocamldep is
wrong

*** xix C compiler

**** handle global.c
iterate over ids Storage.Global with initializer
add Data and GLOBL directives

**** handle tests/compiler/checking/initializer_not_constant.c
issue correct warning

**** handle Codegen for strings, outstring, 
refine Compiler.nw accordingly too

do the OSTRING -> ONAME in Rewrite.ml
and return extra info to generate DATA pseudo
instead of doing it in the middle of codegen

**** fix o5c to at least compile helloc.c
so solved Unary GetRef of String and Call

**** => helloc.c working with 05c/o5a/o5l !! the whole toolchain

*** xix assembler/linker toolchain

**** finish oia v0, enough for simple examples
Ast_asmi.ml with all constructs derived from a.y and lex.c

**** testinfra for hello_linux_arm.exe! tests/assembler/ tests/linker/
and use qemu-arm
(hmm but segfault right now with those ELF binaries :( )
...
*** RELAX add shell/TOPORT/ and builder/TOPORT and start clean?
...
** o5c/ovc/oic in ocaml
o5c -w -I/arm/include -I/sys/include

*** Last

**** port canreach, warnreach, etc. but in separate phase in Check.ml
and remove TOPORT/ corresponding code

**** less: fix more codegen/tests for ovc

call.c, apparently can MOVW $0, -4(SP) directly without
 using intermediate register. when mov something else like
 $1, $2, then use intermediate register, but when zero go directly
 special opti in MIPS because special instruction for it?

break.c, if.c, ...: single BEQ instead of CMP and then JEQ

**** less: fix more codegen/tests for o5c (and ovc)
 array.c: missing GLOBL
  and handle correctly codegen for array.c, multiply by size of int so 12
  not 3 for offset

 enum.c looks ok but use extra register instead of ADDI with immediate

 function.c some reorg on order of arith expr, maybe acom?

 break.c: use Relative PC jump instead of Absolute so hard to compare

**** testinfra on o5l too, on binary output on tests/linker/execgen/ ?

**** less: use debug['X'] to also disable ccom, acom, etc?
do it when I see the need and see o5c output differs from 5c_

**** less: why 5c -S output so different from 5c_ -S on simple example like local.c?
...
** o5l/ovl/oil/ in ocaml

*** FUN port il to ocaml!
or must wait it works in goken (at least riscv32)?

**** LATER Move Layoutv.ml in Layout.ml and make code more polymorphic
by passing down size_of_instructions? can do that
when porting il?


*** resume goken/machines/vi to help debugging xix ovl?
make the syscall emulated on Linux so can run vi
just like I run qemu-mips on my Linux machine.

** o5a/ova/oia/ in ocaml

*** Last

**** RELAX finish comment ported code in 5l/asm.c and optab.c, add the case X:
comment

**** add profile.s test case (multi-archs assembly?)

**** resume vi in goken to test a_out support in ovl?

**** goken/5l (not 5l_) does not need setR12? how does it work?
because $msg(SB) has same value than $setR12(SB) and so does not
require R12 to be set? get an L_CON in aclass() for it ?

**** less: ovl: try also big-endian? why segfault with qemu?

**** port 6a/6l? path towards then Mach-O and PE exec formats
and can test on surface book 2 and my apple mini?

or maybe 8a/8l is easier incremental path? and maybe can
reuse most code when then porting 6a/6l?
...
*** try Windows.nw and orio.nw
tell orio.nw = porting of Windows.nw, tell to read Intro.nw
to understand the program architecture and use
...
*** RELAX xix: check for Unix.openfile in semgrep.jsonnet

*** why 'exit 0' in hellorio.ml is not flagged by 'make check'?
because in tests/ ?
...
*** less: try hide some records like Display.t? use accessor
in public API instead? so can make Display.t an abstract type?
same for Image.t ? or useful to see the record in the sig to actually
...
*** less: use camlidl? or implement low-level stuff that writes
into /dev/draw/x/ directly?

*** LATER client/server with simple marshalling protocol?
