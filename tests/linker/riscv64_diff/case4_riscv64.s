...
// assembler).
//
// Unlike a plain "JMP label" (see case4_check_riscv.s), goken's
// assembler treats an unconditional JAL-with-link as a "call that
// returns" and does NOT drop the code it jumps over as dead, even
// though nothing here ever actually returns to it (R8 is set but
// never used to jump back) -- confirmed by the ADD below surviving
// byte-for-byte in goken's own output.
//
// case 4's plain-JMP-to-label form, and JMP's interaction with
// conditional branches/loops, is deliberately NOT covered by a
// byte-identical fixture -- see case4_check_riscv.s's comment for
// why every natural shape collides with one of goken's own
// assembler-side code-layout optimizations (dead-code-after-JMP
// elision, the branch+jmp inversion already documented in
// case3_riscv.s, and a "loop rotation" that, combined with a later
// JAL/indirect-return in the same function, was observed via an
// ad-hoc test to miscompute a target address into a non-4-byte-
// aligned garbage branch -- well out of scope).
TEXT _start(SB), $0
    MOVW    $1, R10
    JAL     R8, lbl2
