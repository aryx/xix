...
let regs_initial = 
  let arr = Array.make Ai.nb_registers 0 in
  (* note that rRET is not in the list; it can be used! *)
  [Ai.rLINK; Ai.rZERO; (*Av.rPC;*)       (* hardware reseved *)
   Ai.rTMP; Ai.rSB; Ai.rSP; (* linker reserved *)
...
let arith_instr_of_op (op : C.binaryOp) r1 r2 r3 =
  ...
  | C.Arith op ->
     let r2_opt = 
      ...
      (match op with 
      | C.Plus -> Ai.Arith (Ai.ADD (size), Ai.Reg r1, r2_opt, r3)
      | C.Minus -> Ai.Arith (Ai.SUB (size), Ai.Reg r1, r2_opt, r3)
      | C.And -> Ai.Arith (Ai.AND, Ai.Reg r1, r2_opt, r3)
      | C.Or -> Ai.Arith (Ai.OR, Ai.Reg r1, r2_opt, r3)
      | C.Xor -> Ai.Arith (Ai.XOR, Ai.Reg r1, r2_opt, r3)
...
let mov_operand_of_opd entity_of_id (opd : opd) : (Ai.gen, A.ximm) Either.t =
  match opd.opd with
  | ConstI i   -> Right (A.Int i)
  | Register r -> Left (Ai.GReg r)
  | Name (fullname, offset) -> Left (Ai.Entity (entity_of_id fullname offset))
  | Indirect (r, offset) -> Left (Ai.Indirect (r, offset))
  | Addr fullname -> Right (A.Address (entity_of_id fullname 0))

let move_instr_of_opds entity_of_id (move_size : A.move_size) 
     (opd1: opd) (opd2: opd) : Ai.instr =
  match move_size with
  | A.Word ->
     Ai.Move2 (Ai.W__,  
                      (match mov_operand_of_opd entity_of_id opd1 with
                      | Left gen -> Left (Ai.Gen gen)
