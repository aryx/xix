...
# and case34_check_mips.s are deliberately NOT in this list: all
# seven are functionally correct (matching qemu-mips exit codes) but
# not byte-identical, because goken's sched.c hoists real
# instructions into branch/call delay slots (and, separately, pads
# NOPs around certain MUL-result HI/LO read/write transitions and
# MTC1/MFC1 COP1-transfer read/write transitions) instead of what
# this port emits -- see docs/claude_notes/todo_mips_port.org.
...
# and case34_check_mips.s each inherit *both* that BEQ/JMP gap *and*
# their own respective hazard's NOP-padding gap (see mullohi_mips.s's,
# case30_31_mips.s's and case34_mips.s's own comments for what's
# byte-identical there instead). The root cause and the decision not
# to port the scheduler are identical in all seven. Run any of them
