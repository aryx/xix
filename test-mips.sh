...
# four are functionally correct (matching qemu-mips exit codes) but
# not byte-identical, because goken's sched.c hoists real
# instructions into branch/call delay slots instead of the plain
# NOPs this port emits -- see docs/claude_notes/todo_mips_port.org.
...
# slots, not just 4), but the root cause and the decision not to
# port the scheduler are identical in all four. Run any of them
