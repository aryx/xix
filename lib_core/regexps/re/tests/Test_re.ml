...
(* Literal Match *)
let literal_match_tests () =
  Testo.categorize "literal match" [
...
(* Basic Operations *)
let basic_operations_tests () =
  Testo.categorize "basic operations" [
...
(* String, line, word *)
let string_line_word_tests () =
  Testo.categorize "string, line, word" [
...
(* Match semantics *)
let match_semantics_tests () =
  Testo.categorize "match semantics" [
...
(* Group (or submatch) *)
let group_tests () =
  Testo.categorize "group or submatch" [
...
(* Character set *)
let character_set_tests () =
  Testo.categorize "character set" [
...
(* Case modifiers *)
let case_modifiers_tests () =
  Testo.categorize "case modifiers" [
...
(* Fixed bugs *)
let fixed_bugs_tests () =
  Testo.categorize "fixed bugs" [
...
let tests _caps =
  Testo.categorize_suites "re" [
    ...
    string_line_word_tests ();
    match_semantics_tests ();
    group_tests ();
    character_set_tests ();
    case_modifiers_tests ();
    fixed_bugs_tests ();
  ]
